...
function LoadingState(props: PropsWithChildren) {
  return (
...
function SentCommendations({
  commsSent,
}: {
  commsSent?: Array<CommendationWithPeople>;
}) {
  return (
    <Box flexGrow={1}>
...
function RecvCommendations({
  commsRecv,
}: {
  commsRecv?: Array<CommendationWithPeople>;
}) {
  return (
    <Box flexGrow={1}>
      <Typography textAlign={"center"} variant="h5" marginBottom={2}>
...
export default function AdminTeamView({ teams }: { teams: TeamsList }) {
  ...
  useEffect(() => {
    if (teamSelected) {
      fetch(`/api/teams?id=${teamSelected}`).then((res) => {
        res.json().then((j) => {
  ...
  return (
    <Box>
      <Box marginX={2} display="flex" flexDirection="row" justifyContent="space-between">
        {!bigScreen ?
          <FormControl>
            <InputLabel id="type-select-label" sx={{ marginTop: 1 }}>
              Commendation Type
            </InputLabel>
            <Select
        ...
        <FormControl>
          <InputLabel id="team-select-label" sx={{ marginTop: 1 }}>
            Team
          </InputLabel>
          <Select
            label="Team"
            ...
            {exec.teams?.map((currentTeam: Team, index: number) => (
              <MenuItem key={index} value={currentTeam.id}>
                <Box display={"flex"}>
                  ...
                  />
                  <Typography marginY={"auto"} fontWeight={700} marginLeft={2}>
                    {currentTeam.name}
                  </Typography>
                </Box>
              </MenuItem>
      ...
      <Box display={"flex"} paddingX={1}>
        {bigScreen ? (
          <>
            <SentCommendations commsSent={commsSent ?? []} />
            <RecvCommendations commsRecv={commsRecv ?? []} />
          </>
        ) : sentSelected ? (
          <SentCommendations commsSent={commsSent ?? []} />
        ) : (
