...
export default function CommendationForm({
  recipients,
  teamTab,
}: {
  recipients: (MemberWithTeams | TeamWithMembers)[];
  teamTab?: boolean;
}) {
  ...
  useEffect(() => {
    ...
    if (isMemberListItem(recipients)) {
      setRecipients(
        recipients.filter(
          (member) => (member as MemberWithTeams).email !== session?.user?.email
        )
      );
    }
      ...
      >
        <Stack spacing={1}>
          <Typography
            ...
            fontWeight={900}
          >
            Create {teamTab ? "Team" : ""} Commendation
          </Typography>
          <TextField
            sx={{ display: "none" }}
            hidden
